...
use super::{climb::Climb, formation::Formation, region::Region, sector::Sector};
...
impl Crag {
    ...
    async fn description(&self, ctx: &Context<'_>) -> Result<Option<String>> {
        ...
        let result = client
            .query_one("SELECT description FROM climb.crags WHERE id = $1", &[&self.0])
            .await?;
        let value: Option<&str> = result.try_get(0)?;
    ...
    async fn climbs(&self, ctx: &Context<'_>) -> Result<Vec<Climb>> {
        ...
        let rows = client
            .query(
                "SELECT id FROM climb.climbs WHERE crag_id = $1 ORDER BY name",
                &[&self.0],
            )
            .await?;
